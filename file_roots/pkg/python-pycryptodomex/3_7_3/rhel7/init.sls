...
{{ macros.requires(sls_name, pkg_data) }}
    ...
    - sources:
      - salt://{{slspath}}/sources/{{sls_name}}-3.7.0-sphinx.patch
      - salt://{{slspath}}/sources/{{sls_name}}-{{version}}-use_external_libtomcrypt.patch
      - salt://{{slspath}}/sources/{{sls_name}}-{{version}}-cflags.patch
