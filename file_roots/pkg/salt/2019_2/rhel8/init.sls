...
##        - {{ macros.pypi_source(pypi_name, version) }}
      ...
      - salt://{{slspath}}/sources/{{pkg_name}}-{{version}}-tornado4.patch
      - salt://{{slspath}}/sources/{{pkg_name}}-{{version}}-gpg-strbytes.patch
      - salt://{{slspath}}/sources/{{pkg_name}}-{{version}}-rpmsign.patch
